...
class ASLTrainerApp:
    def __init__(self, root):
        ...
        self.models = {
            ...
            "MultiScaleAttentionalCNN": self.build_multi_scale_attentional_model
        }
    ...
    def build_model(self, model_name, input_shape, num_classes, fine_tune):
        ...
        elif model_name == "MultiScaleAttentionalCNN":  
            return self.build_multi_scale_attentional_model(input_shape, num_classes, fine_tune)
        else:
    ...
    def build_multi_scale_attentional_model(self, input_shape, num_classes, fine_tune):
        """
        Builds the proposed Multi-Scale Attentional Sign CNN.
        This model uses two parallel branches:
        - Branch A (Global): Uses larger kernels (7x7, 5x5) to capture the overall hand shape.
        - Branch B (Local): Uses smaller kernels (3x3) to focus on fine details.
        The outputs are concatenated and fed into a squeeze-and-excitation attention block,
        then flattened and passed through dense layers for classification.
        """
        from tensorflow.keras.layers import Input, Conv2D, MaxPooling2D, BatchNormalization, Concatenate, GlobalAveragePooling2D, Dense, Reshape, Multiply, Flatten, Dropout
        inputs = tf.keras.layers.Input(shape=input_shape)
        
        # Branch A: Global features with larger kernels
        branchA = Conv2D(32, (7, 7), strides=2, padding='same', activation='relu')(inputs)
        branchA = BatchNormalization()(branchA)
        branchA = MaxPooling2D(pool_size=(2, 2))(branchA)
        branchA = Conv2D(64, (5, 5), padding='same', activation='relu')(branchA)
        branchA = BatchNormalization()(branchA)
        branchA = MaxPooling2D(pool_size=(2, 2))(branchA)
        branchA = Conv2D(128, (5, 5), padding='same', activation='relu')(branchA)
        branchA = BatchNormalization()(branchA)
        branchA = MaxPooling2D(pool_size=(2, 2))(branchA)
        
        # Branch B: Local features with smaller kernels
        branchB = Conv2D(32, (3, 3), padding='same', activation='relu')(inputs)
        branchB = BatchNormalization()(branchB)
        branchB = MaxPooling2D(pool_size=(2, 2))(branchB)
        branchB = Conv2D(64, (3, 3), padding='same', activation='relu')(branchB)
        branchB = BatchNormalization()(branchB)
        branchB = MaxPooling2D(pool_size=(2, 2))(branchB)
        branchB = Conv2D(128, (3, 3), padding='same', activation='relu')(branchB)
        branchB = BatchNormalization()(branchB)
        branchB = MaxPooling2D(pool_size=(2, 2))(branchB)
        # NEW: Extra pooling to match branchA's spatial dimensions
        branchB = MaxPooling2D(pool_size=(2, 2))(branchB)
        
        # Feature Fusion via Concatenation followed by an attention (SE) block
        merged = Concatenate()([branchA, branchB])
        
        # Squeeze-and-Excitation (channel attention)
        se = GlobalAveragePooling2D()(merged)
        channels = merged.shape[-1]
        se = Dense(channels // 16, activation='relu')(se)
        se = Dense(channels, activation='sigmoid')(se)
        se = Reshape((1, 1, channels))(se)
        attended = Multiply()([merged, se])
        ...
        for model_name in models_to_train:
            ...
            history = model.fit(
                ...
                callbacks=[self.TrainingCallback(self.progress_label, self.progress_bar, 
                                   self.time_label, self.elapsed_label, epochs, self.stop_event, self.log),
                            early_stopping]
            )
    ...
    class TrainingCallback(tf.keras.callbacks.Callback):
        def __init__(self, progress_label, progress_bar, time_label, elapsed_label, total_epochs, stop_event, log_callback):
            super().__init__()
            ...
            self.epoch_times = []
            self.log_callback = log_callback  # New logging callback
