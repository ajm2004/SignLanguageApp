...
# Define model options for the classifier
model_options = {
    ...
    # T1 MODELS
    "MobileNetV2 (T1)": "TrainedBinaryNonLandMarkNewModel/MobileNetV2_model.h5",
    "VGG16 (T1)": "TrainedBinaryNonLandMarkNewModel/VGG16_model.h5",
    "DenseNet121 (T1)": "TrainedBinaryNonLandMarkNewModel/DenseNet121_model.h5",
    "VGG19 (T1)": "TrainedBinaryNonLandMarkNewModel/VGG19_model.h5",
    "Fusion Model (T1)": "TrainedBinaryNonLandMarkNewModel/FusionModel_model.h5",
    "MobileNet (T1)": "TrainedBinaryNonLandMarkNewModel/MobileNet_model.h5",
    "NASNetMobile (T1)": "TrainedBinaryNonLandMarkNewModel/NASNetMobile_model.h5",
...
def update_frame():
    ...
    if hands:
        ...
        if imgCrop.shape[0] > 0 and imgCrop.shape[1] > 0:
            # Check if the selected model is a T1 model.
            is_T1 = "(T1)" in selected_model_var.get()
            
            # --- For the cropped image display ---
            # If NOT a T1 model, overlay landmarks; otherwise just use a plain copy.
            if not is_T1 and 'lmList' in hand:
                lm_list = hand['lmList']
            ...
            # Create a white canvas and resize the binary image for classification
            aspectRatio = h / w
    ...
    # ---------------------------
    # Auto Type Functionality (unchanged)
    # ---------------------------
    ...
    # --- Update Landmarks Panel ---
    # If a T1 model is selected, display the binary image (no landmarks overlay)
    if "(T1)" in selected_model_var.get() and imgWhite_for_display is not None:
        imgLand_rgb = cv2.cvtColor(imgWhite_for_display, cv2.COLOR_GRAY2RGB)
        imgLand_pil = Image.fromarray(imgLand_rgb).resize((FIXED_WIDTH, FIXED_HEIGHT), resample_method)
        imgLand_tk = ImageTk.PhotoImage(image=imgLand_pil)
        landmarks_label.imgtk = imgLand_tk
        landmarks_label.configure(image=imgLand_tk)
    elif imgCrop_landmarked_for_display is not None:
