...
# Enable XLA (Accelerated Linear Algebra) for potential performance improvements
tf.config.optimizer.set_jit(True)

# Enable mixed precision training (if your GPU supports it)
from tensorflow.keras import mixed_precision
mixed_precision.set_global_policy('mixed_float16')

# Enable memory growth for GPUs to allow dynamic allocation (optional but often useful)
...
if gpus:
    for gpu in gpus:
        try:
            ...
            print(f"Could not set memory growth for GPU {gpu}: {e}")
# ---------------------------------------------------------------------
