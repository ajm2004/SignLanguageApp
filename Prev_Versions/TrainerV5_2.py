...
from tkinter import Tk, Label, Button, filedialog, StringVar, DoubleVar, IntVar, OptionMenu, BooleanVar, Listbox, Scrollbar, Text, VERTICAL, WORD, END
...
class ASLTrainerApp:
    def __init__(self, root):
        ...
        self.selected_models = []  # list to store the sequence (max 7)
        ...
        self.models = {
            ...
            "FusionModel": self.build_fusion_model  # <-- New Fusion Model Option
        }
        ...
        # Left Panel: Selected Models (scrollable) and Log Box (scrollable)
        ...
        self.models_listbox = Listbox(listbox_frame, height=8)
        self.models_listbox.pack(side='left', fill='y')
        list_scroll = Scrollbar(listbox_frame, orient=VERTICAL, command=self.models_listbox.yview)
        list_scroll.pack(side='right', fill='y')
        self.models_listbox.config(yscrollcommand=list_scroll.set)
        ...
        # Right Panel: All parameter selections and buttons
        ...
        split_menu = OptionMenu(right_frame, self.train_test_split, *split_options)
        split_menu.pack(pady=5)
        
        # Model selection drop-down and add/remove buttons
        model_select_label = ttk.Label(right_frame, text="Select Model to Add:")
        model_select_label.pack(pady=5)
        model_menu = OptionMenu(right_frame, self.model_choice, *self.models.keys())
    ...
    def add_model(self):
        ...
        if len(self.selected_models) >= 7:
            ...
            self.models_listbox.insert(END, selected)
            self.log(f"Added model '{selected}' to the sequence.")

    def remove_selected_model(self):
        """Remove the selected model from the list."""
        try:
            index = self.models_listbox.curselection()[0]
            model = self.models_listbox.get(index)
            self.models_listbox.delete(index)
            self.selected_models.remove(model)
            self.log(f"Removed model '{model}' from the sequence.")
        except IndexError:
    ...
    def build_fusion_model(self, input_shape, num_classes, fine_tune):
        """
        Build a joint architecture that fuses features from MobileNetV2 and VGG16.
        Both branches share the same input.
        """
        ...
        # Fuse the features from both branches using concatenation
        from tensorflow.keras.layers import Concatenate
        merged = Concatenate()([x1, x2])
        
        # Add fully connected layers for the fused features
    ...
    def _train_model_thread(self):
        ...
        # Parse train/test split selection; mapping split string to validation_split value.
        ...
        for model_name in models_to_train:
            ...
            history = model.fit(
                ...
                epochs=epochs,
                verbose=1,
                callbacks=[self.TrainingCallback(self.progress_label, self.log_text, epochs), early_stopping]
            )
            ...
            # Generate and save Grad-CAM heatmap for a sample image
            try:
    ...
    def perform_cross_validation(self, model_name, epochs, batch_size, learning_rate, fine_tune):
        ...
        self.status_label.config(text=f"Training final model on entire dataset after CV for {model_name}...")
        ...
        final_training_details = {
            ...
            "fine_tune": fine_tune,
            "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "train_val_split": "Training (75%) | Validation (25%)",
    ...
    def generate_and_save_gradcam_heatmap(self, model, sample_img_path, img_size, model_name, results_folder):
        """Generates a Grad-CAM heatmap for a sample image and saves the overlay."""
    ...
    class TrainingCallback(tf.keras.callbacks.Callback):
        def __init__(self, progress_label, log_text, total_epochs):
            super().__init__()
        ...
        def on_epoch_end(self, epoch, logs=None):
            ...
            # Append to log text widget (thread-safe)
            self.log_text.after(0, lambda: self.log_text.insert(END, text + "\n"))
            self.log_text.after(0, lambda: self.log_text.see(END))
