...
import tensorflow_hub as hub  # NEW: Import TF Hub for pre-trained ViT models
...
# -------------------------------------------------------------------
# Custom layers for Vision Transformer (ViT)
# -------------------------------------------------------------------
class Patches(tf.keras.layers.Layer):
    """Extracts patches from the input images."""
...
class PatchEncoder(tf.keras.layers.Layer):
    """Encodes patches by projecting them into a vector space and adding a learned positional embedding."""
...
class ASLTrainerApp:
    def __init__(self, root):
        ...
        # Available models (note: the two new TF Hub ViT options will be handled in build_model)
        self.models = {
            ...
            "ViT": self.build_vit,  # Our custom-built ViT
            "ViT TFHub B16": None,  # Will be handled in build_model
            "ViT TFHub B32": None   # Will be handled in build_model
        }
    ...
    def build_vit(self, input_shape, num_classes):
        """
        Builds a simple Vision Transformer (ViT) model.
        ViT divides the input image into patches, encodes them,
        applies several transformer blocks, and then classifies the image.
        """
        # ViT parameters
        ...
        projection_dim = 64
        num_heads = 4
        transformer_units = [projection_dim * 2, projection_dim]  # MLP layer sizes
        ...
        for _ in range(transformer_layers):
            ...
            mlp_output = tf.keras.layers.Dense(transformer_units[0], activation=tf.nn.gelu)(x3)
            mlp_output = tf.keras.layers.Dropout(dropout_rate)(mlp_output)
            mlp_output = tf.keras.layers.Dense(transformer_units[1], activation=tf.nn.gelu)(mlp_output)
            mlp_output = tf.keras.layers.Dropout(dropout_rate)(mlp_output)
        ...
        features = tf.keras.layers.Dense(128, activation=tf.nn.gelu)(representation)
        features = tf.keras.layers.Dropout(dropout_rate)(features)
        logits = tf.keras.layers.Dense(num_classes, activation='softmax')(features)
        model = tf.keras.Model(inputs=inputs, outputs=logits)
        return model

    def build_vit_tfhub(self, input_shape, num_classes, hub_url, fine_tune):
        """
        Builds a ViT model using a pre-trained TensorFlow Hub feature extractor.
        The hub_url should point to a TF Hub module (e.g., a ViT feature extractor).
        The hub layer is optionally set to be trainable (for fine-tuning).
        """
        inputs = tf.keras.Input(shape=input_shape)
        vit_layer = hub.KerasLayer(hub_url, trainable=fine_tune, input_shape=input_shape)
        x = vit_layer(inputs)
    ...
    def build_model(self, model_name, input_shape, num_classes, fine_tune):
        if model_name == "BasicCNN":
            ...
            return self.build_vit(input_shape, num_classes)
        elif model_name.startswith("ViT TFHub"):
            # Define mapping from model name to TF Hub URL.
            urls = {
                "ViT TFHub B16": "https://tfhub.dev/sayakpaul/vit_b16_fe/1",
                "ViT TFHub B32": "https://tfhub.dev/sayakpaul/vit_b32_fe/1"
            }
            hub_url = urls.get(model_name)
            if hub_url is None:
                raise ValueError("TF Hub URL not found for model: " + model_name)
            return self.build_vit_tfhub(input_shape, num_classes, hub_url, fine_tune)
        else:
            # For all other pre-defined models (e.g., MobileNet, etc.)
    ...
    def _train_model_thread(self):
        ...
        # Adjust image size for different models
        if model_name == "BasicCNN":
            img_size = (64, 64)
        elif model_name in ["Xception", "InceptionV3"]:
            img_size = (299, 299)
        elif model_name in ["ViT", "ViT TFHub B16", "ViT TFHub B32"]:
            img_size = (224, 224)  # Typical size for ViT models
        else:
            img_size = (224, 224)
